...
test('연구와 일반 문서 워크스페이스의 번역 모드를 따로 저장한다', async ({ page }) => {
  ...
  await expect(page.locator("#settings-mode-badge")).toHaveText("연구 모드")
  await expect(page.locator("#setting-translation-mode-scope")).toHaveCount(0)
...
test('설정 카테고리를 키보드로 전환하고 좁은 화면에서 상단 메뉴로 배치한다', async ({ page }) => {
  await mockBaseRoutes(page, { documents: [] })
  await gotoApp(page)
  await page.locator('#sidebar-settings-btn').click()
  ...
  await expect(page.locator('#settings-nav-translation')).toBeFocused()
  await expect(page.locator('#settings-nav-translation')).toHaveAttribute('aria-selected', 'true')
  await expect(page.locator('#tab-translation .settings-category-header h3')).toHaveText('번역')
  await expect(page.locator('#tab-translation')).toBeVisible()
  await expect(page.locator('#setting-target-lang')).toBeVisible()

  await page.setViewportSize({ width: 600, height: 800 })
  await expect.poll(() => page.locator('.settings-nav').evaluate(element => getComputedStyle(element).flexDirection)).toBe('row')
