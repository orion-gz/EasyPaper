...
test('워크스페이스 전환은 홈과 데이터 범위를 바꾸고 문서 종류 필터를 제공한다', async ({ page }) => {
  ...
  await expect(page.locator('.dash-stat-grid')).toHaveCSS('display', 'grid')
  ...
  await expect(page.locator('.dash-root')).not.toContainText('AI 추천 논문')
...
test('좁은 화면에서도 문서 홈 카드가 화면 밖으로 넘치지 않는다', async ({ page }) => {
  ...
  await expect(page.locator('.dash-empty-note').first()).toBeVisible()
  expect(await page.locator('.dash-root').evaluate(el => el.scrollWidth <= el.clientWidth)).toBe(true)
})
