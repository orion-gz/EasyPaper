...
test('Focus 설정을 모드별로 저장하고 슬라이더 활성 상태를 복원한다', async ({ page }) => {
  ...
  await expect(page.locator('#setting-focus-hide-overlays')).toBeDisabled()
  await page.locator('#setting-focus-mode').locator('..').click()
  await page.locator('#setting-focus-hide-overlays').locator('..').click()
  await expect(page.locator('#setting-focus-hide-overlays')).toBeChecked()
  ...
  await expect(page.locator('#setting-focus-blur-value')).toHaveText('1px')
  await page.locator('#setting-focus-dim').focus()
  await page.keyboard.press('Home')
  await expect(page.locator('#setting-focus-dim-value')).toHaveText('0%')
  await page.keyboard.press('ArrowRight')
  await expect(page.locator('#setting-focus-dim-value')).toHaveText('5%')
  await expect(page.locator('#setting-focus-blur-value')).toHaveText('1px')
  ...
  expect(await page.evaluate(() => localStorage.getItem('easypaper_focus_hide_overlays_research'))).toBe('true')
  ...
  await expect(page.locator('#setting-focus-hide-overlays')).not.toBeChecked()
  await page.locator('#close-settings-btn').click()
  await page.reload()
