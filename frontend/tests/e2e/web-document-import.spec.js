...
async function openArticle(page) {
  ...
  await expect(activeReader(page).locator('.article-viewer')).toBeVisible()
}
...
test('저장된 HTML 원문과 번역, 목차, 검색, 원본 폴백을 표시한다', async ({ page }) => {
  ...
  await activeReader(page).locator('#outline-toggle-btn').click(); await expect(activeReader(page).locator('#outline-content')).toContainText('Introduction')
  await activeReader(page).locator('#outline-toggle-btn').click()
  await activeReader(page).locator('[data-tab="original"]').click(); await expect(activeReader(page).locator('.article-original-site')).toContainText('브라우저에서 원본을 열어주세요')
})

test('HTML 선택 하이라이트를 section anchor로 저장하고 다시 열 때 복원한다', async ({ page }) => {
  await openArticle(page)
  const paragraph = activeReader(page).locator('[data-block-id="block-1"]')
  ...
  await activeReader(page).locator('.article-selection-tools [data-action="highlight"]').click()
  ...
  await page.reload(); await page.evaluate(() => { location.hash = '#viewer?id=web-1' }); await expect(activeReader(page).locator('.article-highlight')).toContainText('Unique searchable source text')
})
