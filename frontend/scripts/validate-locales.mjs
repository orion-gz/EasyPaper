...
for (const locale of localeDirs) {
  ...
  for (const namespace of namespaces) {
    ...
    for (const key of keys) {
      ...
      if (/<\/?[a-z][^>]*>/i.test(value)) throw new Error(locale + '/' + namespace + ':' + key + ' contains HTML')
...
for (const namespaceFile of namespaces) {
  ...
  const dynamic = namespace === 'errors' || (namespace === 'common' && key.startsWith('language.'))
...
for (const file of sourceFiles.filter(file => file.endsWith('.js'))) { const rel=relative(frontendRoot,file), source=readFileSync(file,'utf8'); for (const match of source.matchAll(/(['"])((?:\\.|(?!\1)[\s\S])*?)\1/g)) { const value=match[2].replace(/\s+/g,' ').trim(); if(/[가-힣]/.test(value)&&value.length<1000) hardcoded.add(rel+'\0'+value) } }
const baseline = new Set(JSON.parse(readFileSync(join(frontendRoot,'scripts','hardcoded-ui-baseline.json'),'utf8')))
const added = [...hardcoded].filter(value => !baseline.has(value)); if (added.length) throw new Error('new hardcoded Korean UI strings: '+added.slice(0,5).join(' | '))
console.log('Validated '+localeDirs.length+' locales, '+namespaces.length+' namespaces, '+references.size+' references, and hardcoded-string baseline.')
