...
EasyPaper는 학술 PDF 논문을 AI로 번역하고 논문 내용을 기반으로 대화할 수 있는 통합 웹 서비스입니다. 논문을 업로드하면 원문 옆에 AI 번역본이 함께 표시되며, 궁금한 내용을 바로 질문할 수 있습니다. 로컬 Ollama 모델과 외부 API(Gemini, Claude, OpenAI)를 모두 지원합니다.

---

## ⚡ 빠른 시작

명령어 세 줄로 바로 실행할 수 있습니다:
...
서버 구동 후 브라우저에서 `http://localhost:8000` 에 접속하세요.
...
2. **AI 카테고리 자동 태깅** — 업로드 후 AI가 논문 초록과 본문을 분석하여 카테고리 태그(예: `LLM`, `VLM`, `Optimizer`)를 자동으로 부여합니다.
...
EasyPaper 백엔드에는 Google Antigravity CLI(`agy`)를 서브프로세스로 호출하여 번역, 논문 태깅, 채팅을 처리하는 전용 `antigravity` LLM 제공업체 엔진이 포함되어 있습니다.

`antigravity` 제공업체를 사용하려면 아래 단계를 완료하세요:
