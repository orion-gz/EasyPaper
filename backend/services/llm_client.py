...
async def stream_translation(
    ...
    if ignore_math:
        ...
        rules.append(
            ...
            "※ 중요: 등호(=), 더하기(+), 빼기(-), 분수, 시그마(\\sum), 인테그랄(\\int) 등 여러 연산 기호와 변수가 결합된 복잡한 식은 절대로 여러 개의 인라인 수식으로 잘게 쪼개서 감싸지 마세요. 식 전체(예: $f(x) = x_i + y_i$ 또는 $f(x) = \\sum_{i=1}^n x_i$)를 통째로 하나의 인라인($...$) 또는 블록($$...$$) 수식 기호로 감싸서 온전하게 출력해야 합니다. 수식 중간에 기호나 연산자를 수식 기호($) 바깥으로 노출시키지 마세요."
        )
