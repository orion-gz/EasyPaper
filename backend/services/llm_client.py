...
async def stream_translation(
    ...
    if ignore_math:
        ...
        rules.append("수식, 수식 기호, 수학적 변수나 수식 성분(예: x_i, f(x), \alpha, \beta, O(N) 등), 참조 번호 [1], [2,3], Figure N, Table N 레이블은 원문 그대로 유지하세요. 단, 원문에서 일반 텍스트 기호로 노출되는 수학 기호, 수식, 변수명은 웹 뷰어에서 미려하게 렌더링되도록 반드시 LaTeX 인라인 수식 기호인 $...$ (예: $x_i$, $f(x)$, $\alpha$, $O(N)$) 또는 블록 수식 기호인 $$...$$로 적절히 감싸서 출력해 주세요.")
