...
- fix: 문장 집중 모드에서 호버 미리보기의 범위와 위치를 현재 문장에 맞게 제한하고, 키보드 탐색 및 페이지 스크롤 중에도 안정적으로 갱신되도록 개선
- fix: 집중 모드에서 이전 호버 미리보기가 남아 화면을 가리지 않도록 정리

## 1.0.1 - 2026-09-11

- fix: 데스크톱 재시작 시 PDF 파서 의존성 충돌과 백엔드 시작 무한 대기를 방지
- fix: 데스크톱 업데이트 실패 후에도 백엔드가 유지되고 시작 복구가 정상 동작하도록 개선

## 1.0.0 - 2026-09-10

- feat: 문서 분류 확인, 적응형 읽기 전 브리핑, 점진적 장별 요약을 추가해 긴 문서의 읽기 흐름을 개선 (#536, #539, #540, #542, #558)
- feat: 업로드 시 선택적 AI 분류, 문서 정보 메뉴, 인터페이스 배율, 추천 질문 토글과 최신 GPT-6 Astra·Gemini 3.8 Flash 모델 옵션을 추가 (#535, #541, #544, #546, #548, #552, #556)
- feat: 문장 집중 모드와 AI 답변의 메모 내보내기를 추가 (#563, #564)
- fix: PDF 텍스트·오버레이·번역 렌더링, 뷰어 아웃라인 및 메모 바로가기의 배율별 정합성을 개선 (#543, #550, #553, #554, #560)
- fix: 집중 모드의 배경 블러, 문장 격리, 번역 잘림, 텍스트 선명도·범위·키보드 탐색을 개선 (#566, #568, #570, #572-#579)
- fix: 릴리스 보안, Tauri 업데이트 설정 및 Cargo 패키지 버전 동기화를 강화 (#559, #561, #562)

## 2026-08-31

- feat: 네이티브 select를 커스텀 드롭다운으로 교체해 UI 일관성과 선택 상호작용을 개선 (#512)
- fix: 온보딩 언어 선택기를 헤더로 이동해 레이아웃을 정리 (#510)
- fix: 데스크톱 앱에서 Tauri 버전과 변경 이력을 표시하도록 개선 (#509)

## 2026-08-15

- fix: Windows 데스크톱 PDF 파서 설치 오류 코드 2를 해결하고 크로스플랫폼 frozen sidecar 설치 검증을 추가 (#451)

## 2026-08-14

- feat: 워크스페이스 논문 목록에 다양한 정렬 옵션을 추가 (#448)
- fix: 데스크톱 앱에서 PDF 파서 엔진 설치가 정상 동작하도록 복구 (#444)
- fix: 다중 참고문헌 인용 오버레이와 인용 검색 정확도를 개선 (#445, #447)
- fix: 페이지별 읽기 근거를 누적해 독서 분석 정확도를 개선 (#449)
- feat: Antigravity에 Gemini 3.7 Flash 모델을 추가 (#420)
- feat: 라이브러리에 최근 읽은 순 정렬, 컴팩트 카드 보기 및 필터 접기 상태 저장 기능을 추가 (#421, #424, #425)
- fix: 라이브러리 폴더·목록·컴팩트 카드의 스타일, 높이 및 표시 정보를 일관되게 개선 (#422, #427, #428, #430, #433, #440, #442)
- fix: PDF 파서 관리 인증, 문서 삭제 정합성, 세션 사용자명 처리, API 요청 크기 및 프론트엔드 CSP를 강화 (#423, #426, #429, #431, #435)
- fix: 라이브러리 탐색을 포함한 핵심 프론트엔드 E2E 흐름을 복구 (#434, #437)
- perf: 무거운 프론트엔드 워크스페이스 청크를 분리해 초기 번들 크기를 개선 (#439)
- refactor: 백엔드의 deprecated API를 제거하고 프론트엔드 온보딩 모듈을 분리 (#436, #441)
- ci: 백엔드 pytest 및 프론트엔드 E2E 테스트 게이트를 추가 (#432, #438)
